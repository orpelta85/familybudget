...
export function useRunSetup() {
  ...
  return useMutation({
    mutationFn: async (userId: string) => {
      const sb = createClient()
      // Insert profile (trigger may have already done this, so upsert)
      await sb.from('profiles').upsert({ id: userId, display_name: 'אורי' }, { onConflict: 'id' })

      // Insert budget categories
      await sb.from('budget_categories').insert(
        STARTER_CATEGORIES.map(c => ({ ...c, user_id: userId }))
      )

      // Insert sinking funds
      await sb.from('sinking_funds').insert(
        STARTER_FUNDS.map(f => ({ ...f, user_id: userId }))
      )
    },
    onSuccess: (_, userId) => {
