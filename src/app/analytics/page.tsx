          ...
          <div className="bg-[oklch(0.16_0.01_250)] border border-[oklch(0.25_0.01_250)] rounded-xl p-5 mb-4">
              ...
              {sorted.map(([name, amount]) => {
                ...
                return (
                  <div key={name} className="flex items-center gap-3">
                    <span className="text-[12px] text-[oklch(0.70_0.01_250)] w-24 text-end truncate shrink-0">{name}</span>
                    ...
                    <span className="text-[11px] text-[oklch(0.65_0.01_250)] w-16 ltr text-end shrink-0">{formatCurrency(amount)}</span>
                    <span className="text-[10px] text-[oklch(0.55_0.01_250)] w-10 text-start shrink-0">{pct.toFixed(1)}%</span>
                  </div>
                )
              })}
            </div>
            {yearNet > 0 && (
              <div className="flex items-center gap-3 mt-3 pt-3 border-t border-[oklch(0.22_0.01_250)]">
                <span className="text-[12px] text-[oklch(0.70_0.18_145)] w-24 text-end shrink-0 font-semibold">חיסכון</span>
                <div className="flex-1 h-5 rounded-sm bg-[oklch(0.20_0.01_250)] overflow-hidden">
                ...
                <span className="text-[11px] text-[oklch(0.70_0.18_145)] w-16 ltr text-end shrink-0 font-semibold">{formatCurrency(yearNet)}</span>
                <span className="text-[10px] text-[oklch(0.55_0.01_250)] w-10 text-start shrink-0">{((yearNet / yearIncome) * 100).toFixed(1)}%</span>
              </div>
            )}
          </div>
        )
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr className="border-b border-[oklch(0.22_0.01_250)]">
                {['מחזור', 'הכנסות', 'אישי', 'משותף', 'סה"כ הוצ׳', 'תזרים', 'דירה'].map(h => (
                  <th key={h} className="py-2 px-3 text-start text-[oklch(0.65_0.01_250)] font-medium text-[11px] uppercase tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {chartData.map((row, i) => {
                ...
                return (
                  <tr key={i} className={`border-b border-[oklch(0.20_0.01_250)] ${hasData ? 'opacity-100' : 'opacity-35'}`}>
                    ...
                    <td className="py-2 px-3 ltr text-end text-[oklch(0.65_0.18_250)] font-medium">{row.income > 0 ? formatCurrency(row.income) : '—'}</td>
                    <td className="py-2 px-3 ltr text-end">{row.personal > 0 ? formatCurrency(row.personal) : '—'}</td>
                    <td className="py-2 px-3 ltr text-end">{row.shared > 0 ? formatCurrency(row.shared) : '—'}</td>
                    <td className="py-2 px-3 ltr text-end text-[oklch(0.72_0.18_55)]">{row.expenses > 0 ? formatCurrency(row.expenses) : '—'}</td>
                    <td className={`py-2 px-3 ltr text-end font-semibold ${row.net >= 0 ? 'text-[oklch(0.70_0.18_145)]' : 'text-[oklch(0.62_0.22_27)]'}`}>
                      {row.income > 0 ? formatCurrency(row.net) : '—'}
                    </td>
                    <td className="py-2 px-3 ltr text-end text-[oklch(0.70_0.18_145)]">{row.saved > 0 ? formatCurrency(row.saved) : '—'}</td>
                  </tr>
                )
              })}
            </tbody>
            <tfoot>
              <tr className="border-t-2 border-[oklch(0.25_0.01_250)]">
                ...
                <td className="py-2.5 px-3 ltr text-end font-bold text-[oklch(0.65_0.18_250)]">{formatCurrency(yearIncome)}</td>
                <td colSpan={2} />
                <td className="py-2.5 px-3 ltr text-end font-bold text-[oklch(0.72_0.18_55)]">{formatCurrency(yearExpenses)}</td>
                <td className={`py-2.5 px-3 ltr text-end font-bold ${yearNet >= 0 ? 'text-[oklch(0.70_0.18_145)]' : 'text-[oklch(0.62_0.22_27)]'}`}>{formatCurrency(yearNet)}</td>
                <td className="py-2.5 px-3 ltr text-end font-bold text-[oklch(0.70_0.18_145)]">{formatCurrency(yearSaved)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
