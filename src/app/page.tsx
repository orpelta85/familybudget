...
import { Wallet, Receipt, TrendingUp, PiggyBank, Home, AlertTriangle } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const donutData = Object.entries(expByType).map(([type, value]) => ({
    ...
    value,
    color: TYPE_COLORS[type] ?? 'oklch(0.50 0.01 250)',
  }))

  // ── Sinking funds balance ─────────────────────────────────────────────────
  function getFundBalance(fundId: number) {
    const txns = allSinkingTx?.filter(t => t.fund_id === fundId) ?? []
    const dep = txns.filter(t => t.amount > 0).reduce((s, t) => s + t.amount, 0)
    const wdw = txns.filter(t => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0)
    return dep - wdw
  }
    ...
    <div>
      ...
      {showAlert && (
        <div style={{
          ...
          border: '1px solid oklch(0.32 0.10 30)',
          ...
          <div style={{ fontSize: 13, color: 'oklch(0.85 0.05 55)', lineHeight: 1.5 }}>
            {netFlow < 0 && (
              <span>תזרים שלילי החודש ({formatCurrency(netFlow)}). </span>
            )}
            {overspentCats.length > 0 && (
              <span>
                קרובים לתקרה: {overspentCats.map(c => c.name).join(' · ')}
              </span>
            )}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(155px, 1fr))', gap: 12, marginBottom: 16 }}>
        {[
          {
            label: 'הכנסה נטו',
            value: dataLoading ? '—' : formatCurrency(totalIncome),
            color: 'oklch(0.65 0.18 250)',
            Icon: Wallet,
          },
          {
            label: 'הוצאות החודש',
            value: dataLoading ? '—' : formatCurrency(totalExpenses),
            color: 'oklch(0.72 0.18 55)',
            Icon: Receipt,
          },
          {
            label: 'תזרים נקי',
            value: dataLoading ? '—' : formatCurrency(netFlow),
            color: netFlow >= 0 ? 'oklch(0.70 0.18 145)' : 'oklch(0.62 0.22 27)',
            Icon: TrendingUp,
          },
          {
            label: '% חיסכון',
            value: dataLoading ? '—' : `${savingsPct}%`,
          ...
          <div key={kpi.label} style={CARD}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: 'oklch(0.55 0.01 250)', textTransform: 'uppercase', letterSpacing: '0.03em' }}>
                {kpi.label}
              </span>
              <kpi.Icon size={14} style={{ color: kpi.color, opacity: 0.7 }} />
            </div>
            <div style={{ fontSize: 22, fontWeight: 700, color: kpi.color, direction: 'ltr', letterSpacing: '-0.02em' }}>
              {kpi.value}
            </div>
            {kpi.sub && (
              <div style={{ fontSize: 11, color: kpi.color, opacity: 0.7, marginTop: 4 }}>{kpi.sub}</div>
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center', direction: 'ltr' }}>
                        {deviation !== null && (
                          <span style={{
                            fontSize: 10,
                            color: deviation > 15 ? 'oklch(0.62 0.22 27)' : deviation < -15 ? 'oklch(0.70 0.18 145)' : 'oklch(0.50 0.01 250)',
                          }}>
        ...
        <div style={CARD}>
          ...
          {donutData.length === 0
            ? <div style={{ color: 'oklch(0.55 0.01 250)', fontSize: 13 }}>אין נתונים להצגה</div>
            : (
              <>
                  ...
                  <PieChart>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{
                        ...
                        borderRadius: 8, fontSize: 12, color: 'oklch(0.85 0.01 250)',
                      }}
                    />
                  </PieChart>
                </ResponsiveContainer>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontWeight: 600, fontSize: 14 }}>
              <PiggyBank size={14} style={{ color: 'oklch(0.70 0.15 185)' }} />
              קרנות צבירה
            </div>
                ...
                <div key={fund.id} style={{ marginBottom: 11 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                    ...
                    <span style={{
                      ...
                      color: balance >= 0 ? 'oklch(0.70 0.15 185)' : 'oklch(0.62 0.22 27)',
                    }}>
                  ...
                  <div style={{ height: 4, borderRadius: 2, background: 'oklch(0.22 0.01 250)', overflow: 'hidden' }}>
                    <div style={{
                      ...
                      background: 'oklch(0.70 0.15 185)',
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontWeight: 600, marginBottom: 6, fontSize: 14 }}>
            <Home size={14} style={{ color: 'oklch(0.70 0.18 145)' }} />
            יעד הדירה
          </div>
          <div style={{ fontSize: 11, color: 'oklch(0.50 0.01 250)', marginBottom: 16 }}>
            3,500 ₪ × 36 מחזורים = 126,000 ₪
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14 }}>
            <span style={{ fontSize: 26, fontWeight: 700, direction: 'ltr', color: 'oklch(0.70 0.18 145)' }}>
              {formatCurrency(totalSaved)}
            </span>
            <span style={{ fontSize: 12, color: 'oklch(0.55 0.01 250)' }}>
              מתוך {formatCurrency(APARTMENT_TARGET)}
            </span>
          </div>
          <div style={{ height: 7, borderRadius: 4, background: 'oklch(0.22 0.01 250)', overflow: 'hidden', marginBottom: 8 }}>
            <div style={{
              ...
              width: `${apartmentPct}%`,
              background: 'linear-gradient(90deg, oklch(0.55 0.18 145), oklch(0.70 0.18 145))',
              transition: 'width 0.4s ease',
            }} />
          </div>
