    ...
    <div>
      <div className="flex justify-between items-start mb-1.5">
        <div className="flex items-center gap-2">
          <CreditCard size={18} className="text-[var(--c-shared-0-65)]" />
          <h1 className="text-xl font-bold tracking-tight">מנויים</h1>
          <PageInfo {...PAGE_TIPS.subscriptions} />
        </div>
        <button onClick={() => setNewSub({ name: '', amount: '', billingDay: '1', categoryId: '' })} className="btn-hover flex items-center gap-1.5 bg-[var(--c-shared-0-20)] border border-[var(--c-shared-0-32)] rounded-lg px-3.5 py-[7px] text-[var(--c-shared-0-70)] text-[13px] font-medium cursor-pointer">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-xl p-5">
          <div className="text-[11px] text-[var(--text-secondary)] mb-1.5 uppercase tracking-wide">סה"כ חודשי</div>
          <div className="text-2xl font-bold text-[var(--accent-orange)]">{formatCurrency(totalMonthly)}</div>
        </div>
        <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-xl p-5">
          <div className="text-[11px] text-[var(--text-secondary)] mb-1.5 uppercase tracking-wide">צפי שנתי</div>
          <div className="text-2xl font-bold text-[var(--accent-red)]">{formatCurrency(totalYearly)}</div>
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-xl p-5 mb-4">
        ...
        {!activeSubs.length
          ? <div className="text-xs text-[var(--text-secondary)] text-center py-6"><Inbox size={32} className="text-[var(--c-0-30)] mx-auto mb-2" />אין מנויים פעילים</div>
          : activeSubs.map(sub => {
            const isEditing = editingId === sub.id
            const catName = sub.category_id ? categories?.find(c => c.id === sub.category_id)?.name : null
            if (isEditing) {
              return (
                <div key={sub.id} className="py-2.5 border-b border-[var(--c-0-20)] flex flex-col gap-1.5">
                  <div className="flex gap-1.5">
                    <input type="text" value={editForm.name} onChange={e => setEditForm(p => ({ ...p, name: e.target.value }))} className="flex-1 bg-[var(--bg-hover)] border border-[var(--border-light)] rounded-md px-2 py-1 text-[12px] text-inherit" />
                    <input type="number" value={editForm.amount} onChange={e => setEditForm(p => ({ ...p, amount: e.target.value }))} className="w-20 bg-[var(--bg-hover)] border border-[var(--border-light)] rounded-md px-2 py-1 text-[12px] text-inherit ltr text-left" />
                    <input type="number" value={editForm.billingDay} onChange={e => setEditForm(p => ({ ...p, billingDay: e.target.value }))} min="1" max="31" className="w-14 bg-[var(--bg-hover)] border border-[var(--border-light)] rounded-md px-2 py-1 text-[12px] text-inherit ltr text-left" placeholder="יום" />
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => handleSaveEdit(sub.id)} className="flex items-center gap-1 bg-[var(--c-shared-0-65)] text-[var(--c-0-10)] border-none rounded-md px-2 py-1 text-[11px] font-semibold cursor-pointer"><Check size={11} /> שמור</button>
                    <button onClick={() => setEditingId(null)} className="bg-transparent border border-[var(--border-light)] text-[var(--text-secondary)] rounded-md px-2 py-1 text-[11px] cursor-pointer">ביטול</button>
                  </div>
                </div>
              )
            }
            return (
              <div key={sub.id} className="flex justify-between items-center py-2.5 border-b border-[var(--c-0-20)]">
                <div>
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-[13px] font-semibold text-[var(--accent-orange)]">{formatCurrency(sub.amount)}</span>
                  <button onClick={() => startEdit(sub)} aria-label="ערוך" className="bg-transparent border-none cursor-pointer p-1.5 text-[var(--c-0-45)]"><Pencil size={11} /></button>
                  <button onClick={() => handleToggleActive(sub.id, sub.is_active)} aria-label="השהה" className="bg-transparent border-none cursor-pointer p-1.5 text-[var(--c-0-45)]"><Pause size={11} /></button>
                  <button onClick={() => handleDelete(sub.id)} aria-label="מחק" className="bg-transparent border-none cursor-pointer p-1.5 text-[var(--c-0-45)]"><Trash2 size={11} /></button>
                </div>
              </div>
            )
          })
      ...
      {inactiveSubs.length > 0 && (
        <div className="bg-[var(--c-0-14)] border border-[var(--bg-hover)] rounded-xl p-5 opacity-70">
          <div className="font-bold text-sm mb-3 text-[var(--text-secondary)]">מנויים מושהים ({inactiveSubs.length})</div>
          {inactiveSubs.map(sub => (
            <div key={sub.id} className="flex justify-between items-center py-2 border-b border-[var(--c-0-20)]">
              <span className="text-[13px] text-[var(--text-muted)]">{sub.name}</span>
              <div className="flex items-center gap-1.5">
                <span className="text-[13px] text-[var(--text-muted)]">{formatCurrency(sub.amount)}</span>
                <button onClick={() => handleToggleActive(sub.id, sub.is_active)} aria-label="הפעל" className="bg-transparent border-none cursor-pointer p-1.5 text-[var(--accent-green)]"><Play size={11} /></button>
                <button onClick={() => handleDelete(sub.id)} aria-label="מחק" className="bg-transparent border-none cursor-pointer p-1.5 text-[var(--c-0-45)]"><Trash2 size={11} /></button>
              </div>
            </div>
          ))}
      ...
      {newSub && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-[var(--c-0-18)] border border-[var(--border-light)] rounded-[14px] p-7 w-[360px]">
            ...
            <button onClick={handleAdd} disabled={addSub.isPending || !newSub.name.trim() || Number(newSub.amount) <= 0} className={`w-full bg-[var(--c-shared-0-65)] border-none rounded-lg py-[11px] font-semibold text-sm text-[var(--c-0-10)] ${addSub.isPending ? 'cursor-not-allowed opacity-50' : 'cursor-pointer opacity-100'}`}>
